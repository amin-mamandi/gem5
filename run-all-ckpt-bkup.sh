...
CKPT_BASE_DIR="$GIT_ROOT/ckpts-palloc"
...
# Check if ckpt-bkup.sh exists and is executable
if [ ! -x "./ckpt-bkup.sh" ]; then
    echo "Error: ckpt-bkup.sh not found or not executable"
    exit 1
fi
...
run_simulations() {
    ...
    for script in "${scripts[@]}"; do
        ...
        ckpt_dir_param="--checkpoint-dir ${GIT_ROOT}/ckpts-palloc/${script%.sh}"
        
        ./ckpt-bkup.sh $COMMON_PARAMS $bank_params --script-dir "$script_dir" --script "$script" $ckpt_dir_param $latency_param &
        pid=$!
...
run_with_concurrency_limit() {
    ...
    for script in "${scripts[@]}"; do
        ...
        # Run the script with the appropriate directory
        ./ckpt-bkup.sh $COMMON_PARAMS $bank_params --script-dir "$script_dir" --script "$script" &
        pid=$!
        echo "Started process with PID: $pid"
