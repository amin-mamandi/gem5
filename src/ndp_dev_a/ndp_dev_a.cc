...
namespace gem5
{
        ...
        NDPDevA::readPI(uint64_t ridx)
        {
                ...
                {
                case 0:
                        DPRINTF(NDPDevAPI, "NDP device PI: r[%lu] = %lu (addr_data)\n", ridx, pi_addr_data);
                        return pi_addr_data;
                case 1:
                        DPRINTF(NDPDevAPI, "NDP device PI: r[%lu] = %lu (data_size)\n", ridx, pi_data_size);
                        return pi_data_size;
                case 2:
                        DPRINTF(NDPDevAPI, "NDP device PI: r[%lu] = %lu (data_skey)\n", ridx, pi_data_skey);
                        return pi_data_skey;
                case 3:
                        DPRINTF(NDPDevAPI, "NDP device PI: r[%lu] = %lu (cmmd_code)\n", ridx, pi_cmmd_code);
                        return pi_cmmd_code;
                case 4:
                        DPRINTF(NDPDevAPI, "NDP device PI: r[%lu] = %lu (scale_factor)\n", ridx, scaleFactor);
                        return scaleFactor;
                case 5:
                        DPRINTF(NDPDevAPI, "NDP device PI: r[%lu] = %lu (stat_rgst)\n", ridx, pi_stat_rgst);
                        return pi_stat_rgst;
                case 6:
                        DPRINTF(NDPDevAPI, "NDP device PI: r[%lu] = %lu (last_rslt)\n", ridx, pi_last_rslt);
                        return pi_last_rslt;
                default:
        ...
        NDPDevA::writePI(uint64_t ridx, uint64_t data)
        {
                ...
                case 4:
                        // Only check if busy when trying to start processing
                        if (!pi_stat_rgst)
                        {
