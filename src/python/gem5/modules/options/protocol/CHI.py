class CHI_NODE:
    def __init__(self, parameters, link_latency=0):
        ...
        self.nodes_per_router = (
            parameters["nodes_per_router"]
            if "nodes_per_router" in parameters
            else None
        )
        self.link_latency = (
            parameters["link_latency"]
            if "link_latency" in parameters
            else link_latency
        )
...
class Options:
    def __init__(self, parameters, num_cpus=0, num_slcs=0, link_latency=0):
        ...
        self.nodes["RNF"].nodes_per_router = num_cpus // len(
            self.nodes["RNF"].router_list
        )
        self.nodes["HNF"].nodes_per_router = num_slcs // len(
            self.nodes["HNF"].router_list
        )
