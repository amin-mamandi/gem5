...
class MemoryRegion:
    def __init__(self, parameters):
        ...
        if self.ctrl or self.model == "BwLatCtrl":
            self.ctrl_config = {
                "sampling_window": parameters["ctrl_config"][
                    "sampling_window"
                ],
...
class CacheController:
    def __init__(self, parameters):
        ...
        self.alloc_on_seq_line_write = parameters.get(
        ...
        self.dealloc_backinv_unique = parameters.get(
            "dealloc_backinv_unique", True
        )
        self.dealloc_backinv_shared = parameters.get(
...
class CacheOptions:
    def __init__(self, parameters, is_icache=False):
        ...
        if ("prefetcher" in parameters) and (
...
class NOC:
    def __init__(self, parameters, num_cpus=None, num_slcs=None):
        ...
        protocol_name = parameters["protocol"][
            "model"
        ]  # m5.defines.buildEnv['PROTOCOL']
        protocol_params = parameters["protocol"][f"{protocol_name}"]
        Protocol = importlib.import_module(
            f"gem5.modules.options.protocol.{protocol_name}"
        )

        self.protocol = Protocol.Options(
            protocol_params,
            num_cpus=num_cpus,
            num_slcs=num_slcs,
            link_latency=self.topology.node_link_latency,
        )
...
class Architecture:
    def __init__(self, parameters, checkpoint_restore=False):
        ...
        self.num_slcs = (
            self.num_cpus * parameters["system"]["num_slcs_per_cpu"]
        )
        ...
        parameters["caches"]["SLC"]["bus-width"] = int(
            util.divide_float_unitless(
                parameters["caches"]["SLC"]["bandwidth"],
                parameters["system"]["clock"],
            )
        )

        self.cpu = CPU(
            parameters["cpu"], checkpoint_restore=self.checkpoint_restore
        )
        self.caches = Caches(parameters["caches"])
        self.system = System(parameters["system"])
        self.memory = Memory(parameters["memory"])
        self.NOC = NOC(
            parameters["NOC"], num_cpus=self.num_cpus, num_slcs=self.num_slcs
        )
...
class Options:
    def __init__(self):
        ...
        self.architecture = Architecture(
            self.parameters, checkpoint_restore=self.checkpoint_restore
        )
...
def add_options(parser):
    ...
    parser.add_argument(
        "--config-file", type=str, default=None, help="Configuration file"
    )
    ...
    parser.add_argument(
        "--dtb-filename", type=str, default=None, help="DTB file to load"
    )
    parser.add_argument(
        "--kernel", type=str, default=None, help="Linux kernel"
    )
    parser.add_argument(
        "--disk",
        action="append",
    ...
    parser.add_argument(
        "--bootloader", type=str, default=None, help="Bootloader path"
    )
    parser.add_argument(
        "--root-partition",
        type=str,
    ...
    parser.add_argument(
        "--bootscript", type=str, default=None, help="Linux bootscript"
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Instantiate Checkpoint Parameters",
    )
    parser.add_argument(
        "--checkpoint-restore",
        "-r",
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="The command to run in syscall emulation mode.",
    )
    parser.add_argument(
        "--cmd-env",
        type=str,
        default="",
        help="Initialize command environment from text file.",
    )
    parser.add_argument(
        "--cmd-input", type=str, default="", help="Read stdin from a file."
    )
    parser.add_argument(
        "--cmd-output", type=str, default="", help="Redirect stdout to a file."
    )
    parser.add_argument(
        "--cmd-outerr", type=str, default="", help="Redirect stderr to a file."
    )
