...
class EPISystemSE(m5.objects.System):
    def __init__(self, options, **kwargs):
        ...
        self.mem_ranges = [
            m5.objects.AddrRange(options.architecture.memory.size)
        ]
        ...
        for i in range(options.architecture.num_cpus):
            ...
            for j in range(self.cpu[i].numThreads):
                self.cpu[i].isa[j].sve_vl_se = (
                    int(options.architecture.cpu.sve_length) / 128
                )
    ...
    def configure_caches(self, options):
        from gem5.modules.caches import (
            L1D,
            L1I,
            L2,
            L3Slice,
        ...
        if last_cache_level == 3:
            bus_width_L2_L3_slice = options.parameters["caches"]["SLC"][
                "bus-width"
            ]
            L3_slice_per_core = options.parameters["caches"]["SLC"][
                "slices_per_core"
            ]
            ...
            for cpu in self.cpu:
                cpu.addTwoLevelCacheHierarchy(
                    ...
                    WalkCache(options.parameters["caches"]["WalkCache"]),
                    m5.objects.L2XBar(
                        width=L2_xbar_width, clk_domain=self.cpu_clk_domain
                    ),
                )
            mem_size = options.architecture.memory.size
            mem_range = m5.objects.AddrRange(
                "2GB", size="2GB"
            )  # self.realview._mem_regions[0]
    ...
    def configure_memory(self, options):
        from gem5.modules.memory import (
            create_memory_interface,  # HBM, HBM2, DDR4_2400_8x8
        )
        ...
        for r in self.mem_ranges:
            for i in range(mem_channels):
                dram_iface = create_memory_interface(
                    r, i, mem_channels, options
                )
