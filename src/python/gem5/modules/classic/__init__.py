...
def configure_caches(self, options):
    ...
    if last_cache_level == 3:
        ...
        system_mem_range = m5.objects.AddrRange(
            start=mem_range.start, size=mem_size
        )
        ...
        for match in range(0, pow(2, n_bits)):
            l3_aux = L3Slice(
                options.parameters["caches"]["SLC"], clk_domain=self.clk_domain
            )
...
def configure_classic(
    system, options, piobus=None, dma_ports=[], bootmem=None
):
    ...
    if options.architecture.caches.cache_levels > 0:
        ...
        system.iobridge = m5.objects.Bridge(
            delay="50ns", ranges=system.mem_ranges
        )
