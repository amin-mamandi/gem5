...
CKPT_BASE_DIR="$GIT_ROOT/ckpts-sync-3"
...
# Check if ckpt-copy.sh exists and is executable
if [ ! -x "./ckpt-copy.sh" ]; then
    echo "Error: ckpt-copy.sh not found or not executable"
    exit 1
fi
...
run_simulations() {
    ...
    for script in "${scripts[@]}"; do
        ...
        ckpt_dir_param="--checkpoint-dir ${GIT_ROOT}/ckpts-sync-3/${script%.sh}"
        
        ./ckpt-copy.sh $COMMON_PARAMS $bank_params --script-dir "$script_dir" --script "$script" $ckpt_dir_param $latency_param &
        pid=$!
...
run_with_concurrency_limit() {
    ...
    for script in "${scripts[@]}"; do
        ...
        # Run the script with the appropriate directory
        ./ckpt-copy.sh $COMMON_PARAMS $bank_params --script-dir "$script_dir" --script "$script" &
        pid=$!
        echo "Started process with PID: $pid"
